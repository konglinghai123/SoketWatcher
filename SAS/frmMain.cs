    ...
    //-----------------------------------------------------
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            if (Common.load())
            {
                ...
                startconnect();
                dt = DateTime.Now;
               //  textBox8.Text = textBox8.Text+string.Format("{0:T}", dt)+"\r\n";//14:23:23 ;
                //timer1.Start();
                
            }
            else
            {
                MessageBox.Show("找不到数据库?","系统提示");
            }
            fm.Text = fm.Text + "[" + dt.ToShortDateString().ToString() + "]";//----显示日期
        ...
        private void startconnect()
        {
            ...
            {
                MessageBox.Show("数据库没有数据,请你添加","系统提示");
        ...
        private void 查询ToolStripMenuItem_Click(object sender, EventArgs e)
        {
          if (listView1.CheckedItems.Count==1)
          {
              if (listView1.CheckedItems[0].SubItems[2].Text=="在线")
              {
                    ...
                    {
                        MessageBox.Show("正在等待上一次的返回结果，请稍后","系统提示");
                    }
              ...
              {
                  MessageBox.Show("请选择在线设备","系统提示");
              }
          } 
          else
          {
              MessageBox.Show("没有选中设备", "系统提示");
          }
        }
  ...
  /*      public void MsgToServer()
            ...
            if (listView1.CheckedItems.Count == 1)
            {
                if (listView1.CheckedItems[0].SubItems[2].Text == "在线")
                {
                     ...
                     else
                     {
                         ...
                         MessageBox.Show("正在等待上一次操作的返回结果","系统提示");
        ...
        {  
            ...
            for (int i = 0; i < listView1.Items.Count;i++ )
            {
                ...
                {
                    MessageBox.Show(ex.ToString(), "系统警告！");
                }
        ...
        {
            if (MessageBox.Show("重启软件吗？", "系统提示", MessageBoxButtons.OKCancel) == DialogResult.OK)
            { GengXinSohw(); }
           
        }
       public void GengXinSohw() 
        {
            client.CloseThread();          
            Application.Restart();
            //------------------------
...
*/
       ...
       private void run(Object obj)
       {
        ...
        {   
            ...
            if (fig == true)
            {
                MessageBox.Show("设防数据成功发到[" + Mif + "]","系统提示");

                strMsg = "[设防成功]-\r\n";
            }
            else
            {
                MessageBox.Show("设防[" + Mif + "]失败","系统提示");
                strMsg = "[设防失败]-\r\n"; 
...
//------------------------------------------------------------
        ...
        private void backgroundWorker3_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (fig == true)
            {
                MessageBox.Show("成功关闭[" + Mif + "]警报声", "系统提示");
                strMsg = "[成功关闭警报声音]";
            }
            else
            {
                MessageBox.Show("关闭[" + Mif + "]警报声失败", "系统提示");
                strMsg = "[关闭警报失败,超时?]";
            }
            //--------------------------------------------------

            MessageInfo info = new MessageInfo(strMsg, listView1.CheckedItems[0].SubItems[1].Text, "开关操作", DateTime.Now.ToString());
            insert.insert(info);
        }
        ...
        private void backgroundWorker4_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (fig == true)
            {
                MessageBox.Show("成功打开[" + Mif + "]警报声","系统提示");
                strMsg = "[成功打开警报声音]";
            }
            else
            {
                MessageBox.Show("打开[" + Mif + "]警报声失败!", "系统提示");
                strMsg = "[打开警报失败!超时?]";
            }
            //----------------------------------------------
        ...
        private void listView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (listView1.SelectedIndices != null && listView1.SelectedIndices.Count > 0)
            {
                ...
                string ts = "";
                //-----------------------------门禁延时--------
                si1 = listView1.Items[c[0]].SubItems[14].Text;
                if (si1 != "")
                {
                    textBox4.Text = si1;
                }
                ...
                //-----------------------------向铃声时间--------
                si1 = listView1.Items[c[0]].SubItems[13].Text;
                if (si1 != "")
                {
                    textBox3.Text = si1;
                }
               ...
               // else { checkBox13.Checked = false; }
                si1 = listView1.Items[c[0]].SubItems[6].Text;
                if (si1.IndexOf(">") != -1) { checkBox13.Checked = true; }
                else { checkBox13.Checked = false; }
                for (int i = 7; i < 13; i++)
                {
